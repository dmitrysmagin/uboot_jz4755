...
static int dualboot_mode_check(void)
{
...
#define CFG_NAND_MINIOS_OFFS 0x100000
#define CFG_NAND_MINIOS_SIZE 0x100000
#define CFG_NAND_MINIOS_START 0x80800000

	__gpio_as_input(UBOOT_SEL_DUALBOOT_KEY);

	if( __gpio_get_pin(UBOOT_SEL_DUALBOOT_KEY) ==1)
	{
		nand_load(CFG_NAND_MINIOS_OFFS, CFG_NAND_MINIOS_SIZE,
			(uchar *)CFG_NAND_MINIOS_START);
		uboot = (void (*)(void))CFG_NAND_MINIOS_START;	
	}
	else
	{
	...
	//nand_load(CFG_NAND_U_BOOT_OFFS, CFG_NAND_U_BOOT_SIZE,
	//		(uchar *)CFG_NAND_U_BOOT_DST);
	//nand_load(0x100000, 0x100000,
			//(uchar *)0x80800000);

	//uboot = (void (*)(void))CFG_NAND_U_BOOT_START;
	//uboot = (void (*)(void))0x80800000;
